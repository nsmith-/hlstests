...
int main(void)
{
  ...
  for(size_t i=0; i<1000; ++i) {
    ...
    lorentz::eta_t deta = std::rand()*1./RAND_MAX;
    lorentz::phi_t dphi = std::rand()*1./RAND_MAX;

    input[1].pt  = lorentz::xyzt_t(std::rand()*190./RAND_MAX+10.);
    input[1].eta = lorentz::eta_t(input[0].eta+deta);
    input[1].phi = lorentz::phi_t(input[0].phi+dphi);
    ...
    hls_demo(input, output, m2);

    if ( i < nprint ) std::cout << "output " << output << ", mass2 = " << m2 << std::endl;
    ...
    double fp_m2 = (std::max(pow(E0+E1,2)-pow(px0+px1,2)-pow(py0+py1,2)-pow(pz0+pz1,2),0.));
    ...
    double d_M = ((double) m2 - fp_m2) / lsb;
